...
| SoPilot X起爆帖 | **YouMind X爆款** | `python3 scripts/youmind_viral_scraper.py --proxy http://127.0.0.1:1080 --limit 20` |
| 知乎热榜（已失效） | 百度热搜 | `python3 -c "import urllib.request,re; ..."` |
| 微博热搜（已失效） | 百度热搜 | 同上 |
...
| 组合 | 覆盖率 |
|------|--------|
| 热点刀锋（YouMind + 百度热搜） | ~65%（海外爆款+国内热点） |
| + Buzzing.cc | ~80%（+28个海外媒体聚合） |
| + HN Firebase | ~90%（+科技垂直） |
